...
function NeuralNet3D({ active }: { active: boolean }) {
  ...
  useEffect(() => {
    ...
    for (let li = 0; li < NET.length; li++) {
      ...
      for (let n = 0; n < NET[li]; n++) {
        ...
        scene.add(Object.assign(new THREE.Mesh(sphereGeo, mat), { position: pos }))

        const glowMat = new THREE.MeshBasicMaterial({
          color: 0x00d4ff, transparent: true, opacity: 0, depthWrite: false, side: THREE.BackSide,
        })
        scene.add(Object.assign(new THREE.Mesh(glowGeo, glowMat), { position: pos }))

        const outerMat = new THREE.MeshBasicMaterial({
          color: 0x00d4ff, transparent: true, opacity: 0, depthWrite: false, side: THREE.BackSide,
        })
        scene.add(Object.assign(new THREE.Mesh(outerGeo, outerMat), { position: pos }))
  ...
  }, [active])   // ← re-runs when slide becomes active
